...
## G. 补充澄清（2026-09-19）：60mm 基线 + MJPEG 组合问题

**结论：60mm + MJPEG 只能用于链路冒烟，不能用于精度定案与正式数据；采购底线维持基线 ≥80mm（最好 120mm）+ 无压缩 MONO8/YUY2。**

60mm 基线量化影响（路线 A 参数 f=3.6mm，δd=0.15px）：

| 模式 | f_px | δz@2m | δz@2.5m | δz@3.5m |
|---|---|---|---|---|
| 640×400/眼 @120fps | 600 | 16.7mm ✗ | 26mm ✗ | 51mm ✗✗ |
| 1280×720/眼 @60fps | 1200 | 8.3mm ✓ | 13mm ✓ | 25mm ✗ |

120fps 模式下 60mm 连 2m 都不达 15mm，只能退 60fps——失去 120fps 核心意义（120mm 基线 120fps@2m 为 9–11mm ✓）。

MJPEG 的本质问题是精度而非带宽：① JPEG 8×8 块效应与运动边缘振铃污染亚像素立体匹配（δd=0.1–0.15px 假设不成立），非线性噪声不可标定消除；② ChArUco 角点亚像素提取被涂抹，标定残差上升；③ "120fps 仅 MJPEG" 意味该档位无无压缩流，属白牌排除项；④ 120fps 双流解码持续占 ~0.5–1 核，与推理抢 CPU。

## H. 补充澄清（2026-09-19）：模组接 MacBook 调试/训练 batana-core

UVC 免驱，MacBook（Type-C 直插）可作为：① core 管线/标定/3D 重建的算法调试主机；② 移动数据采集站（录像落盘 + session-schema 打包，服务 M1.5 数据集）；③ 训练（PyTorch MPS）。限制：TFLite→RKNN 转换需 Linux 环境（rknn-toolkit2 macOS arm64 支持待验证）；帧配对/PTS/长时掉帧等性能验收必须在 RK3588/泰山派目标平台做，MacBook 结果不作数。已知坑：macOS UVC 对 MONO8/GREY 枚举不稳定，高帧率模式可能只暴露 MJPEG——到手先探测（ffmpeg avfoundation 格式列表），兜底用 YUY2 取 Y 通道。
